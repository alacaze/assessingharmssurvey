...
The commit for the current draft is `r system('git rev-parse --short HEAD', intern = TRUE)`, version `r system('git describe --tags', intern = TRUE)` (`r system('git log -1 --format=%cd --date=short', intern = TRUE)`).
...
Nazanin is has responsibility for overallseeing the project and supervising Anvir.
...
Hoekstra and colleagues adopted the questionnaire to focus on infernces regarding confidence intervals rather than $p$-values.
@Hoekstra:2014hf observed results consistent with the previous studies focusing on $p$-values and also suggested that misinterpretation of confidence intervals was comment regardless of experience or education.
Researchers and statistical educators misinterpreted confidence interval at similar rates to students [@Hoekstra:2014hf].
Hoekstra and colleagues findings suggest that 
Though confidence intervals were proposed to attenuate the tendency to dichotomize evidence, Hoekstra et al.^1^ concluded that confidence intervals are employed as an alternative method to significance testing.
